...
from .models import ComprehensionQuestion, Phrase, Prompt, Response, Theme
...
def retired_scope_url(scope):
    ...
    if not ee3 and scope.get("response"):
        ee3 = Response.objects.filter(
            pk=scope["response"], theme__task__part__slug="ee",
            theme__task__slug="tache-3",
        ).exists()
    if not ee3 and scope.get("prompt"):
        ee3 = Prompt.objects.filter(
            pk=scope["prompt"], theme__task__part__slug="ee",
            theme__task__slug="tache-3",
        ).exists()
    if not ee3 and scope.get("theme"):
        ee3 = Theme.objects.filter(
